...
function App() {
	...
	const createNewCard = (message) => {
		axios
			.post(`${kBaseUrl}/boards/${selectedBoard.board_id}/cards`, {
				message,
			})
			.then((response) => {
	...
	const updateLikes = async (cardId) => {
		try {
			await axios.patch(
				`${kBaseUrl}/boards/${selectedBoard.board_id}/cards/${cardId}`
			);
			onCardLikes(cardId);
		} catch (err) {
			console.error(err);
				...
				<section className="upperGrid">
					...
					<section>
						...
						<div
							className="deleteBoards"
							onClick={(event) =>
								deleteOneBoard(selectedBoard.board_id)
							}
						>
							{selectedBoard.board_id ? "Delete This Board" : ""}
						</div>
